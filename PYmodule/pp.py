...
omega = mtd['TOPAZ_44752.nxs'].getRun()['omega'].value[0]
omega_modified = float(omega)
print('Sample Goniometer Omega = {: 8.3f}'.format(omega))
proton_charge = mtd['TOPAZ_44752.nxs'].getRun().getProtonCharge() * 0.0036  # get proton charge

print("\nSample ", str(run), " has integrated proton charge of", proton_charge, "\n")

Rebin(InputWorkspace='TOPAZ_44752.nxs', OutputWorkspace='TOPAZ_44752.nxs', Params='1000,10,16600')
ConvertToMD(InputWorkspace='TOPAZ_44752.nxs', QDimensions='Q3D', dEAnalysisMode='Elastic', Q3DFrames='Q_lab', 
    LorentzCorrection=True, Uproj='1,0,0', Vproj='0,1,0', Wproj='0,0,1', 
    OutputWorkspace='TOPAZ_44752_md', MinValues='-12.5,-12.5,-12.5', MaxValues='12.5,12.5,12.5')

BinMD(InputWorkspace='TOPAZ_44752_md', AxisAligned=False, 
    ...
    OutputBins='501,501,501', OutputWorkspace='TOPAZ_44752_md_3D')
...
R = mtd['TOPAZ_44752_md_3D_mask'].getExperimentInfo(0).run().getGoniometer().getR()
...
# File path
file_path = os.path.join(output_folder, 'data_empty_44752_5C.npy')

# TOPAZ/IPTS-28924 No sample run 47994 with 85.4 C of proton charge
# Save the array
np.save(file_path, data_qsample.astype(np.float32))

data_loaded=np.load(output_folder+'data_empty_44752_5C.npy')
...
# Save mask
# File path
file_path_mask = os.path.join(output_folder, 'mask_empty_44752_bool_5C.npy')
